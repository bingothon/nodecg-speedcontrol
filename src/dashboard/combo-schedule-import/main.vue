...
{
	"en": {
		"panelTitle": "Combo Schedule Import",
...
export default class extends Vue {
	...
	async loadSchedule(): Promise<void> {
		try {
			const data = await nodecg.sendMessage<HoraroSchedule>('loadComboSchedule', {
				url: this.url,
				dashID: this.dashID,
			});
			this.loaded = true;
	...
	async import(confirm: boolean): Promise<void> {
		if (confirm) {
			try {
				await nodecg.sendMessage('importComboSchedule', {
