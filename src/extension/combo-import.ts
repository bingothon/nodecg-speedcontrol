...
async function importOengusPlayers(marathonShort: string,
  scheduleSlug: string) {
  try {
    oengusImportStatus.value.importing = true;
    const scheduleResp =
            await get(`/marathons/${marathonShort}/schedules/for-slug/${scheduleSlug}`);
    ...
    const playersArray = await mapSeries(
      oengusLines
        .filter((line: OengusLine | undefined) =>
          line && line.game && !checkGameAgainstIgnoreList(line.game, 'oengus'))
      ...
      async (line: OengusLine, index: number, arr: any) => {
        ...
        const players = await mapSeries(profiles, async (profile) => {
          if (!profile) {
          ...
          if (!config.oengus.disableSpeedrunComLookup) {
            const playerTwitter = profile.connections
              ?.find((c) => c.platform === 'TWITTER')?.username;
            const playerSrcom = profile.connections
 ...
 */
async function importSchedule(optsO: ImportOptions,
  dashID: string,
  oengusShort: string,
  scheduleSlug: string): Promise<void> {
  try {
    ...
    const newRunDataArray = await mapSeries(runItems.filter((run) => (
      ...
      const gameTwitch = parseMarkdown(run.data[opts.columns.gameTwitch]).str;
      ...
      const allOengusPlayers = await importOengusPlayers(oengusShort, scheduleSlug);
          ...
          async (rawTeam: any) => {
            ...
            team.players = await mapSeries(
              ...
              async (rawPlayer: any) => {
                ...
                const oengusPlayer = allOengusPlayers.find((player: {
                  name: string,
                  displayName: string
                }) => player.name === str || player.displayName === str);
