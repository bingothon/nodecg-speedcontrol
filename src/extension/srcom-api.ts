 ...
 */
export async function searchForTwitchGame(
  query: string,
  abbr = false
): Promise<string> {
  try {
    ...
    if (abbr) {
      const resp = await get(
        `/games?abbreviation=${encodeURIComponent(query)}&max=1`
      );
      ...
      const resp1 = await get(
        `/games?name=${encodeURIComponent(query)}&max=10`
      );
      const results1 = resp1.body.data as Speedruncom.GameData[];
      let exact1 = results1.find(
        (game) => game.names.international.toLowerCase() === query.toLowerCase()
      );
      // If no exact match, use unsupported API to see if that has one.
      if (!exact1) {
        try {
          const resp2 = await get(
            `/ajax_search.php?term=${encodeURIComponent(query)}`
          );
          const results2 = (resp2.body as Speedruncom.AjaxSearch[]).filter(
            (r) => r.category === 'Games'
          );
          const exact2 = results2.find(
            (game) => game.label.toLowerCase() === query.toLowerCase()
          );
          // If it does have an exact match, look that one up on the regular API.
          if (exact2) {
            const resp3 = await get(
              `/games?abbreviation=${encodeURIComponent(exact2.url)}&max=1`
            );
    ...
    nodecg.log.debug(
      `[speedrun.com] Twitch game name lookup failed for "${query}":`,
      err
    );
    throw err;
  }
...
export async function searchForUserData({
  ...
  type: 'name' | 'srcom' | 'twitch' | 'twitter';
  val: string;
  ...
  try {
    ...
    if (data.pronouns) {
      ...
      if (
        !split.includes('he/him') &&
        !split.includes('she/her') &&
        !split.includes('they/them')
      ) {
        data.pronouns = '';
      }
    }
    ...
    nodecg.log.debug(
      `[speedrun.com] User data lookup failed for "${type}/${val}":`,
      err
    );
    throw err;
  }
